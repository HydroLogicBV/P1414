...
class HyDAMO:
    ...
    def __init__(self, extent_file: Union[Path, str] = None) -> None:
        ...
        self.profile_roughness = ExtendedDataFrame(required_columns=["code", "profielpuntid"])
    ...
    def generate_nodes_with_bedlevels(
        ...
        for order, (branch, nodes) in tqdm(
            ...
            if order == -1:
                ...
                while n < n_length:
                    up = (
                        np.count_nonzero(all_nodes[n][0] == np.array([x[0] for x in all_nodes]))
                        == 1
                    )
                    down = (
                        np.count_nonzero(all_nodes[n][1] == np.array([x[1] for x in all_nodes]))
                        == 1
                    )
                    ...
                    if n == n_length:
                        up = all(
                            [
                                np.count_nonzero(node[0] == np.array([x[0] for x in all_nodes]))
                                == 1
                        ...
                        down = all(
                            [
                                np.count_nonzero(node[1] == np.array([x[1] for x in all_nodes]))
                                == 1
                                for node in all_nodes
                            ]
                        )
        ...
        node_geom = [
            Point(x, y)
            for x, y in zip(self.mesh1d.get_values("nnodex"), self.mesh1d.get_values("nnodey"))
        ]
        if resolve_at_bifurcation_method == "min":
            upstream_bedlevel = [
                np.min(v["up"]) if len(v["up"]) > 0 else np.nan for v in nodes_dict.values()
            ]
            downstream_bedlevel = [
            ...
            upstream_bedlevel = [
                np.max(v["up"]) if len(v["up"]) > 0 else np.nan for v in nodes_dict.values()
            ]
            downstream_bedlevel = [
    ...
    def parametrised_to_profiles(
        ...
        for param in parametrised.itertuples():
            ...
            if pd.isnull(
                ...
                print(
                    "bodemhoogte bovenstrooms not available for profile {}.".format(param.globalid)
                )

            # Determine characteristics
            botlev = (
                values[values.soortparameter == "bodemhoogte benedenstrooms"].waarde.values[0]
                + values[values.soortparameter == "bodemhoogte benedenstrooms"].waarde.values[0]
            ) / 2.0
            ...
            if pd.isnull(
                ...
                css_type = "trapezium"
                dh1 = (
                    values[values.soortparameter == "hoogte insteek linkerzijde"].waarde.values[0]
                    - botlev
                )
                dh2 = (
                    values[values.soortparameter == "hoogte insteek rechterzijde"].waarde.values[0]
                    - botlev
                )
                height = (dh1 + dh2) / 2.0
...
class ExternalForcings:
    ...
    def add_lateral(
        ...
        if isinstance(discharge, pd.Series):
            times = ((discharge.index - discharge.index[0]).total_seconds() / 60.0).tolist()
...
class Structures:
    ...
    def check_branchid_chainage(self, branchid, chainage):
        ...
        if not branchid in self.hydamo.branches["code"]:
            raise ValueError(f"branchid {branchid} not present. Give an existing branch.")
...
class ObservationPoints:
    ...
    def add_points(
        ...
        if self.observation_points.empty:
            ...
            self.observation_points = self.observation_points.append(obs, ignore_index=True)
