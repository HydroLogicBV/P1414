...
class MetadataGenerator():
    ...
    def execute(self):
        ...
        for case in cases:
            ...
            if not self.load_structures_ini(self.inputdir_case + 'dflowfm\structures.ini'):
                self.logger.error('Error when loading structures.ini file')
                return False 

            # Step 4: Open the NetCDF DFM_his file
            if not self.load_dfm_his(self.outputdir_case + 'dflowfm\output\DFM_his.nc'):
                self.logger.error('Error when loading dfm_his.nc file')
                return False

            # Step 5: Open the mdu dfm file
            if not self.load_dfm_mdu(self.inputdir_case + 'dflowfm\DFM.mdu'):
                self.logger.error('Error when loading dfm.mdu file')
                return False

            # Step 6: Open the DFM dia file
            if not self.load_dfm_dia(self.outputdir_case + 'dflowfm\output\DFM.dia'):
...
if __name__ == "__main__":
    ...
    metadatagenerator = MetadataGenerator(r"C:\Work\Projects\HL-P24050_ROI\05_Analysis\SAS_workdir\run_model\test_dambreach", r"C:\Work\Projects\HL-P24050_ROI\05_Analysis\SAS_workdir\run_model\test_dambreach")
