...
def build_dhydro(
    ...
    twod_depth_path: str = None,
):

    hydamo = HyDAMO()
    if not "waterloop" in features:
    ...
    hydamo.culverts.snap_to_branch(hydamo.branches, snap_method="ends", maxdist=max_snap_dist)
    ...
    if twod:
        ...
        mesh.mesh2d_add_rectilinear(
            network=network, polygon=extent.geometry.values[0], dx=100, dy=100
        )
    ...
    dimr.component.append(
        FMComponent(
            name="DFM", workingDir=Path(output_folder) / "fm", model=fm, inputfile=fm.filepath
        )
    )
