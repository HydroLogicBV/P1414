...
def _generate_primal(G, gdf_network, fields, multigraph, decimals):
    """
    ...
    for row in gdf_network.itertuples():
        ...
        first = (np.around(first[0], decimals), np.around(first[1], decimals))
        last = (np.around(last[0], decimals), np.around(last[1], decimals))
...
def gdf_to_nx(
    ...
    if approach == "primal":
        _generate_primal(net, gdf_network, fields, multigraph, decimals=decimals)
