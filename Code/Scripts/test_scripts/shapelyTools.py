...
def bend_towards(line, where, to):
    ...
    for k, vertex in enumerate(coords):
        if where.almost_equals(Point(vertex)):
            ...
            coords[k] = to.coords[0][:2] # Added [:2] to deal with 3D lines/points
...
def snap_endpoints(base_lines_mls: shapely.geometry.MultiLineString,
                   snap_to_mls: shapely.geometry.MultiLineString,
                   max_dist: int = 5):
    
    """Snap the isolated endpoints from one MultiLineString to a target MultiLineString.
    
    Args:
        base_lines_mls (MultiLineString): the dataset from which the endpoints need to be snapped
        snap_to_mls (MultiLineString): the target dataset to which the endpoints of base_lines_mls are snapped
        max_distance: maximum distance to search for the nearest point to snap to
    ...
    # only move isolated endpoints, one by one
    for endpoint in isolated_endpoints_base:
    # find all vertices within a radius of max_distance as possible
        target = nearest_neighbor_within(snapping_points_target, endpoint, 
    ...
    # find the LineString to modify within snapped_lines and update it        
        for i, snapped_line in enumerate(base_lines):
            if endpoint.touches(snapped_line):
                try:
                    base_lines[i] = bend_towards(snapped_line, where=endpoint, 
                                                        to = target)
                except: print(f'Dimensions mismatch. Endpoint coords {endpoint.coords[0]}, target coords {target.coords[0]}')

    # post-processing: remove any resulting lines of length 0
    base_lines = [s for s in base_lines if s.length > 0]

    return shapely.geometry.MultiLineString(base_lines)

def MultiLineString_to_shp(inputdata: list,outputfilename: str):
    temp_gdf = gpd.GeoDataFrame(inputdata)
    temp_gdf = temp_gdf.rename(columns={0: 'geom'})
    temp_gdf = temp_gdf.set_geometry('geom')
    temp_gdf.to_file(outputfilename)